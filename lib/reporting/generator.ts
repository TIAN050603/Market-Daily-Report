...
export async function generateMarketReport(date = getTodayDate()) {
  ...
  const generated: GeneratedReport = {
    ...
    macro: {
      ...
      source_urls: [{ title: "BEA schedule", url: "https://www.bea.gov/news/schedule" }]
    },
    decliners,
    watchlist: buildWatchlist(date),
    sources: sourceRows(rankedNews, date)
  };
